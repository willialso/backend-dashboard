...
EXPIRY_LABELS: Dict[int,str] = {
    5: "5 Minute", 15: "15 Minute", 30: "30 Minute", 60: "1 Hour",
    120: "2 Hour", 240: "4 Hour", 480: "8 Hour", 1440: "1 Day"
}

# === PRICING & VOLATILITY ENGINE ===
RISK_FREE_RATE = 0.05
MIN_VOLATILITY = 0.15
MAX_VOLATILITY = 3.00
...
VOLATILITY_SHORT_EXPIRY_ADJUSTMENTS = {5:1.50, 15:1.30, 30:1.15, 60:1.05, 120:1.02}
...
BOT_OTM_MAX_FACTORS = {"Beginner":0.07, "Intermediate":0.05, "Advanced":0.03}

TRADE_SIMULATION_INTERVAL_SECONDS = 30  # seconds between bot loops

# Added default success rate
TRADER_SUCCESS_RATE = 0.7  # fallback if not overridden per type

# === HEDGE FEED & POSITION MANAGER ===
...
HEDGE_EXECUTION_TIMES_MS = {
    EXCHANGE_NAME_COINBASE_PRO: 180,
    EXCHANGE_NAME_KRAKEN:       220,
    EXCHANGE_NAME_OKX:          150,
    EXCHANGE_NAME_DERIBIT:      120,
    "Simulated Internal":       10
}
MAX_RECENT_HEDGES_LOG_SIZE = 150

# === LIQUIDITY MANAGER ===
LM_INITIAL_TOTAL_POOL_USD      = 1_500_000.0
LM_INITIAL_ACTIVE_USERS        = BASE_TOTAL_SIMULATED_TRADERS
LM_BASE_LIQUIDITY_PER_USER_USD = 10_000.0
LM_VOLUME_FACTOR_PER_USER_USD  = 500.0
LM_OPTIONS_EXPOSURE_FACTOR     = 0.25
LM_STRESS_TEST_BUFFER_PCT      = 0.20
LM_MIN_LIQUIDITY_RATIO         = 0.20
LM_MAX_LIQUIDITY_RATIO         = 0.80
MIN_LIQUIDITY_RATIO            = 0.20  # for audit
MAX_LIQUIDITY_RATIO            = 0.80  # for audit
...
MAX_ERROR_RATE_PCT             = 5.0    # new
MAX_DOWNTIME_PCT               = 1.0    # new
    ...
    if key.isupper() and isinstance(current_value, (str, int, float, bool, list, dict)):
        ...
        if env_value_str is not None:
            try:
                ...
                config_logger.info(f"CONFIG: Overrode '{key}' with env var '{env_key}' = '{new_value}'")
            except (ValueError, TypeError, json.JSONDecodeError) as e:
                config_logger.warning(f"CONFIG: Could not cast {env_key}='{env_value_str}' to {type(current_value)}: {e}")
